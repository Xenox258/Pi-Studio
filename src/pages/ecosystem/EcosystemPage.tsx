...
import { beginCatalogLoading, catalogModeLoading, setCatalogNavigationPending, type CatalogMode } from '../../stores/catalogLoadingStore';
...
function loadCatalog(mode: Mode): PackageInfo[] | Promise<PackageInfo[]> {
  ...
  const request = (async () => {
    ...
    if (generation === catalogGeneration) catalogCache.set(mode, packages);
    return packages;
  })();
...
function DiscoverCard(props: { item: PackageInfo; selected: boolean; busy: string; onSelect: () => void; onInstall: () => void }) {
  ...
  return <button class={`discover-card ${props.selected ? 'is-selected' : ''}`} onClick={() => props.onSelect()}>
    <div class="discover-card__head"><PackageGlyph item={props.item} /><span><strong>{props.item.name}</strong><em>{props.item.author || 'Unknown author'}</em></span><Badge tone="accent">{props.item.kind}</Badge></div>
