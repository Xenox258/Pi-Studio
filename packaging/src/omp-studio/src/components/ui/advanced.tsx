...
export function Popover(props: { trigger: JSX.Element; children: JSX.Element; label: string }) {
  ...
  return <span class="popover"><button class="popover__trigger" aria-haspopup="dialog" aria-expanded={open()} onClick={() => setOpen(!open())}>{props.trigger}</button><Show when={open()}><span class="popover__content" role="dialog" aria-label={props.label}>{props.children}</span></Show></span>;
}
