...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct StoredSession { pub id: String, pub title: String, pub project_id: String, pub updated_at: String, pub active: bool }
...
impl Database {
    ...
    pub fn save_session(&self, id: &str, project_id: &str, title: &str) -> StudioResult<()> {
        self.with_connection(|connection| { connection.execute("INSERT INTO sessions(id,project_id,title,updated_at) VALUES (?1,?2,?3,?4) ON CONFLICT(id) DO UPDATE SET title=excluded.title, updated_at=excluded.updated_at", params![id, project_id, title, Utc::now().to_rfc3339()])?; Ok(()) })
    }

    pub fn recent_sessions(&self, active_id: Option<&str>) -> StudioResult<Vec<StoredSession>> {
        self.with_connection(|connection| { let mut statement = connection.prepare("SELECT id,title,project_id,updated_at FROM sessions ORDER BY updated_at DESC LIMIT 100")?; let rows = statement.query_map([], |row| { let id: String = row.get(0)?; Ok(StoredSession { active: active_id == Some(id.as_str()), id, title: row.get(1)?, project_id: row.get(2)?, updated_at: row.get(3)? }) })?; rows.collect::<Result<Vec<_>, _>>().map_err(StudioError::from) })
    }
...
mod tests {
    ...
    #[test] fn migrates_and_persists_history() { let database = Database::open(Path::new(":memory:")).unwrap(); database.save_project("project-1", "Project", "/tmp/project").unwrap(); database.save_session("session-1", "project-1", "Project").unwrap(); assert_eq!(database.recent_projects().unwrap()[0].id, "project-1"); let sessions = database.recent_sessions(Some("session-1")).unwrap(); assert_eq!(sessions[0].project_id, "project-1"); assert!(sessions[0].active); let version: u32 = database.with_connection(|connection| Ok(connection.pragma_query_value(None, "user_version", |row| row.get(0))?)).unwrap(); assert_eq!(version, 1); }
